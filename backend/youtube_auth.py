...
# Store flow object between auth URL generation and callback
_active_flow = None
...
def handle_callback(code: str):
    ...
    if _active_flow is None:
        # Recreate flow if server restarted between auth and callback
        _active_flow = Flow.from_client_secrets_file(
            CLIENT_SECRET, scopes=SCOPES,
            redirect_uri=REDIRECT_URI
        )
...
def upload_video(filepath, title, description, tags,
                 category_id='22', privacy='private', publish_at=None):
